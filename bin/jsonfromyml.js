...
const dataFiles = glob.sync(path.join(__dirname, "../testfiles/test*.yml"));
...
const indivDir = path.join(__dirname, "../build/testfiles");
if (!fs.existsSync(indivDir)) fs.mkdirSync(indivDir);

// ---------------------------------------- | Index Page

// Path to the index page in the build dir.
const indexPath = path.join(buildDir, "test.json");
