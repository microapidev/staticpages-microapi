...
class UserContoller {
  ...
  async signUp(req, res) {
    ...
    if (!fullName || !email || !password)
      throw new CustomError("Please fill in the required field");
  ...
  async updateConfig(req, res) {
    ...
    if (!user) {
      res.status(404).send(response("user dosen't exist", null, false));
      throw new CustomError("user dosen't exist", 404);
    }

    res.status(200).send(response("All Files Found", user.config, true));
  }
}
